...
phase1_system_preparation() {
    ...
    apt-get install -y -qq \
        ...
        build-essential || {
            ...
            apt-get install -y -qq curl wget ca-certificates usbutils || {
...
phase2_gnome() {
    print_header "PHASE 2: GNOME DISPLAY MANAGER"
    
    log "Installing GNOME and GDM3..."
    DEBIAN_FRONTEND=noninteractive apt-get install -y -qq \
        gnome-session \
        gdm3 \
        gnome-shell \
        gnome-settings-daemon || {
            log_error "Failed to install GNOME"
            exit 1
        }
    ...
    if [ -f "$gdm_conf" ]; then
        # Backup original
        cp "$gdm_conf" "${gdm_conf}.backup-$(date +%Y%m%d)"
        
        # Clean approach: remove ALL auto-login entries first, then add fresh ones
        log "Removing any existing auto-login configuration..."
        sed -i '/^AutomaticLoginEnable/d' "$gdm_conf"
        sed -i '/^AutomaticLogin[^E]/d' "$gdm_conf"
        
        # Check if [daemon] section exists
        if grep -q "^\[daemon\]" "$gdm_conf"; then
            # Section exists, add our config right after it
            sed -i "/^\[daemon\]/a AutomaticLogin = $DEVICE_USER" "$gdm_conf"
            ...
            echo "AutomaticLogin = $DEVICE_USER" >> "$gdm_conf"
        fi
        
        log "GDM3 configured for auto-login as: $DEVICE_USER"
        log_info "Verifying configuration..."
        if grep -A 2 "^\[daemon\]" "$gdm_conf" | grep -q "AutomaticLogin = $DEVICE_USER"; then
            log "✓ Auto-login verification passed"
        else
            log_warning "⚠ Auto-login configuration may need manual review"
        fi
    else
        log_error "GDM3 config file not found"
        exit 1
    fi
    
    log "Creating GNOME settings script..."
    cat > /usr/local/bin/gastro-gnome-settings.sh <<'GNOME_EOF'
#!/bin/bash
# GNOME Settings for Kiosk Mode
# Must run as the logged-in user
...
# Auto-hide top bar (optional)
# gsettings set org.gnome.shell.extensions.dash-to-dock autohide true
GNOME_EOF
    
    chmod +x /usr/local/bin/gastro-gnome-settings.sh
    
    log "Setting GDM3 as default display manager..."
    ...
    # Create marker file for existing user
    mkdir -p /home/$DEVICE_USER/.config
    touch /home/$DEVICE_USER/.config/gnome-initial-setup-done
    chown -R $DEVICE_USER:$DEVICE_USER /home/$DEVICE_USER/.config
    
    # Create marker file for future users
    mkdir -p /etc/skel/.config
    touch /etc/skel/.config/gnome-initial-setup-done
    
    # Remove autostart entries if they exist
    rm -f /etc/xdg/autostart/gnome-initial-setup*.desktop 2>/dev/null || true
    rm -f /etc/xdg/autostart/gnome-welcome-tour.desktop 2>/dev/null || true
    
    # Disable systemd services if they exist
    systemctl disable gnome-initial-setup-first-login.service 2>/dev/null || true
    systemctl disable gnome-initial-setup-copy-worker.service 2>/dev/null || true
    systemctl mask gnome-initial-setup-first-login.service 2>/dev/null || true
    
    log "✓ GNOME welcome screen disabled"
    
    log "Disabling GNOME Keyring password prompt for auto-login..."
    # For auto-login users, GNOME Keyring should not prompt for password
    # This prevents "Enter password for default keyring" popup on Chromium start
    
    # Method 1: Set empty password for default keyring (will be set on first login)
    mkdir -p /home/$DEVICE_USER/.local/share/keyrings
    cat > /home/$DEVICE_USER/.local/share/keyrings/Default_keyring.keyring <<KEYRING_EOF
[keyring]
...
KEYRING_EOF
    ...
    # Method 2: Disable keyring completely via autostart override
    mkdir -p /home/$DEVICE_USER/.config/autostart
    cat > /home/$DEVICE_USER/.config/autostart/gnome-keyring-secrets.desktop <<'KEYRING_DISABLE_EOF'
[Desktop Entry]
Type=Application
...
KEYRING_DISABLE_EOF
    ...
    # Method 3: Set PAM to unlock keyring automatically (no password needed)
    # This is the cleanest approach - keyring unlocks automatically on auto-login
    if ! grep -q "pam_gnome_keyring.so" /etc/pam.d/gdm-password 2>/dev/null; then
        echo "# Auto-unlock GNOME Keyring for auto-login" >> /etc/pam.d/gdm-password
    ...
    chown -R $DEVICE_USER:$DEVICE_USER /home/$DEVICE_USER/.local 2>/dev/null || true
    chown -R $DEVICE_USER:$DEVICE_USER /home/$DEVICE_USER/.config
    
    log "✓ GNOME Keyring configured for passwordless auto-login"
...
/usr/local/bin/gastro-gnome-settings.sh 2>&1 | tee -a "$LOG_FILE"

# Wait for display server (Wayland or X11)
log "Waiting for display server..."
TIMEOUT=60
COUNTER=0
while ! [ -n "$WAYLAND_DISPLAY" ] && ! xset q &>/dev/null; do
    sleep 1
    ((COUNTER++))
    if [[ $COUNTER -ge $TIMEOUT ]]; then
        log_error "Display server timeout after ${TIMEOUT}s - TRIGGERING HARD RESTART"
        log_error "Attempting to restart GDM3..."
        systemctl restart gdm3
        sleep 10
        
        # If still not working, reboot the machine
        if ! [ -n "$WAYLAND_DISPLAY" ] && ! xset q &>/dev/null; then
            log_error "GDM3 restart failed - HARD REBOOTING SYSTEM"
            sync
            reboot -f
        fi
        break
...
if [ -n "$WAYLAND_DISPLAY" ]; then
    log "Wayland display server ready: $WAYLAND_DISPLAY"
elif xset q &>/dev/null; then
    log "X11 display server ready: $DISPLAY"
else
    log_error "Display server check inconclusive, proceeding anyway..."
fi
...
# IMPORTANT: Set your terminal TID (8 digits)
...
# Terminal IP Address (for direct binding - usually 10.42.0.75)
...
ENV_EOF
    ...
    # REMOVED: Create README (already in GitHub as README.md)
    : <<'README_EOF'
# Payment Terminal Service - Manual Configuration Required

This is a **skeleton installation**. To complete the setup:

## Step 1: Copy PeP Protocol Files from admin1

```bash
# On admin1 (100.64.0.6)
cd ~/payment-terminal-service
tar czf /tmp/pep-protocol.tar.gz src/ server.js .env

# Copy to new device
scp admin1@100.64.0.6:/tmp/pep-protocol.tar.gz /tmp/

# On new device
cd ~/payment-terminal-service
tar xzf /tmp/pep-protocol.tar.gz
```
...
# Set:
# TERMINAL_TID=01100460 (your terminal TID - 8 digits)
# DEVICE_ID=your-hostname
# TERMINAL_IP=10.42.0.75 (or your terminal IP)
```
...
# Check terminal MAC address (should be visible in network)
arp -a | grep 10.42.0.75
```
...
sudo systemctl status gastro-terminal.service

# Check logs
journalctl -u gastro-terminal.service -f
```

## Files Needed from admin1:
- `src/terminal/client.js` - Terminal UDP client
- `src/protocol/packet.js` - PeP packet builder
- `src/protocol/tlv.js` - TLV encoding (BCD support)
- `src/utils/*` - Utility functions
- `server.js` - Full server with PeP protocol
- `.env` - Configuration (copy and modify TID)
