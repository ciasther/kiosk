...
phase2_gnome() {
    ...
    if [ -f "$gdm_conf" ]; then
        ...
        if grep -q "^\[daemon\]" "$gdm_conf"; then
            ...
            sed -i '/^AutomaticLoginEnable/!b;n;/^AutomaticLoginEnable/d' "$gdm_conf"
            sed -i '/^AutomaticLogin/!b;n;/^AutomaticLogin/d' "$gdm_conf"
        else
        ...
        log "GDM3 configured for auto-login"
    else
...
EOF
    ...
    "$PRINTER_DIR/venv/bin/pip" install python-escpos pillow --quiet
